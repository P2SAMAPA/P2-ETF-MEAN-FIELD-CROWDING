"""
Data loading and preprocessing for Mean-Field Crowding engine.
"""
...
def prepare_volume_matrix(df_wide: pd.DataFrame, tickers: list) -> pd.DataFrame:
    """
    Extract volume data for tickers (assume column names like 'Volume_<ticker>' or use price proxy).
    Since master_data may not have volume, we use dollar volume = close * volume if available.
    If not, we approximate using daily volatility as a proxy for activity.
    """
    available_tickers = [t for t in tickers if t in df_wide.columns]
    # For simplicity, we use absolute returns as a volume proxy (activity measure)
